    ...
    while (entries.next()) |list| {
        for (list.items, 0..) |l1, idx1| {
            for (list.items[idx1 + 1 ..]) |l2| {
                ...
                for (0..input_str.len) |multiplier_u| {
                    const multiplier: isize = @intCast(multiplier_u);
                    const l3i = (1 + multiplier) * l1i - multiplier * l2i;
                    const l3j = (1 + multiplier) * l1j - multiplier * l2j;
                ...
                for (0..input_str.len) |multiplier_u| {
                    const multiplier: isize = @intCast(multiplier_u);
                    const l4i = (1 + multiplier) * l2i - multiplier * l1i;
                    const l4j = (1 + multiplier) * l2j - multiplier * l1j;
